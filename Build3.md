...
> - 排序原则：先前端基建后页面消费、先后端契约后前端联调、先数据层后控制层、先依赖后独立。
...
| Step | 内容 | 设计依据 | 状态 |
|------|------|---------|------|
| 1 | 前端共享基建（`api.ts`/`constants.ts`/`types.ts`/`useDryRun.ts`） | Design2 §8.5 | ☐ 未开始 |
| 2 | 后端 Dry Run 升级（明细化/包装/限速/快照锁/防重入）+ 连接测试凭据提示 | Design2 §6.6/§6.7 | ☐ 未开始 |
| 3 | 运行测试页（`RunTest.vue` + `DryRunResults.vue` + 路由/菜单） | Design2 §8.5 | ☐ 未开始 |
| 4 | 页面收敛（Settings/Targets/Dashboard 接入 `api.ts`，删除 Advanced 页） | Design2 §8.6/§8.7 | ☐ 未开始 |
| 5 | 数据层：`Config.SyncEnabled` + SQLite/`.env` 读写 | Design2 §三/§四 | ☐ 未开始 |
| 6 | Syncer 暂停门控（Pause Gate + 热重载开关同步） | Design2 §五 | ☐ 未开始 |
| 7 | API 层：`pause`/`resume` 端点 + trigger 保护 + `SyncStatus.enabled` | Design2 §6.1-6.5 | ☐ 未开始 |
| 8 | 启动决策统一（`main.go`/`app.go` 始终启动 `Run()`） | Design2 §7 | ☐ 未开始 |
| 9 | Dashboard 同步开关 + 按钮状态联动 | Design2 §8.1-8.3 | ☐ 未开始 |
| 10 | 文档同步（Design1/AGENTS/README/.env.example） | Design2 §12.1 | ☐ 未开始 |
| 11 | 低优先可选附项（12.4 A-D，可独立执行） | Design2 §12.4 | ☐ 未开始（可选） |
...
| Step | 涉及文件 | 要点 |
|------|---------|------|
| 1 | `webui/frontend/src/api.ts`（新增）、`constants.ts`（新增）、`types.ts`、`composables/useDryRun.ts`（新增） | 统一 fetch 封装（res.ok 检查 + error 提取 + AbortSignal）；`cloudOptions` 共享常量；`DryRunResult`/`RuleChange`/响应包装/`SyncStatus.enabled?`/`TestConnectionResult` 类型 |
| 2 | `provider/common.go`、`syncer/syncer.go`、`webui/api/sync.go`、`webui/api/targets.go`、`provider/common_test.go`（测试） | `RuleChange` 摘要构造；`DryRun()` 明细化 + 限速 + 快照锁 + 防重入 + warnings；handler 透传包装；凭据空值快速失败（+4 行 + `strings` import） |
| 3 | `components/DryRunResults.vue`（新增）、`views/RunTest.vue`（新增）、`main.ts`、`App.vue` | `NTabs` 双子标签（Dry Run 三级分组视图 + 连接测试表单含 15s 超时）；路由 `/run-test` + 菜单「运行测试」 |
| 4 | `views/Settings.vue`、`views/Targets.vue`、`views/Rules.vue`、`views/Logs.vue`、`views/Dashboard.vue`、`views/Advanced.vue`（删除）、`main.ts`、`App.vue` | 全站接入 `api.ts`（修复 5 处"失败误报成功"）；Targets 弹窗测试连接保留 + 超时；Dashboard 移除试运行 + 「运行测试」链接 + B5 预留；删除 `/advanced` 路由/菜单/文件 |
| 5 | `config/config.go`、`config/store.go`、`config/env.go`、`config/env_test.go`（测试） | `Config.SyncEnabled`；`LoadConfig()` 读 `sync_enabled`；`ParseEnv()` 读 `SYNC_ENABLED` |
| 6 | `syncer/syncer.go`、`syncer/syncer_test.go`（测试） | `syncEnabled` 镜像 + `pauseCh`/`resumeCh` + `Pause()`/`Resume()`/`IsEnabled()` + `Run()` 启动门控 + `waitForResume()` + `Reload()` 开关同步（5.6） |
| 7 | `webui/api/deps.go`、`webui/api/sync.go`、`syncer/syncer.go` | `Syncer` 接口 +`Pause`/`Resume`；`POST /api/sync/pause|resume`；`handleSyncTrigger` 暂停 409；`SyncStatus.Enabled` |
| 8 | `main.go`、`app/app.go` | 确认始终 `go s.Run()`（门控在 Run() 内）；编译/双模式运行验证 |
| 9 | `views/Dashboard.vue`、`types.ts` | 暂停/开启开关按钮；「立即同步」disabled 联动；三态状态标签 |
| 10 | `Design1.md`、`AGENTS.md`、`README.md`、`.env.example`、`docker-compose.yml.example` | 12.9 措辞；十一节新增端点/页面/开关说明；SYNC_ENABLED；端口/页面文档 |
| 11 | `config/env.go`/`config/store.go`、`views/Settings.vue`、`main.go` | 附项 A-D（见 Step 11 详述） |
...
```
Step 1 (前端共享基建: api.ts + constants.ts + types.ts + useDryRun.ts)
  ├─ Step 2 (后端 DryRun 升级 + 凭据提示) ── 依赖 Step 1 的接口约定（types 先行）
  │    └─ Step 3 (运行测试页) ── 依赖 Step 1, 2
  │         └─ Step 4 (页面收敛: 全站 api.ts + 删 Advanced) ── 依赖 Step 1, 3
  └─ Step 5 (数据层: SyncEnabled) ── 无依赖，可与 Step 1-4 线性交错
       └─ Step 6 (Syncer 暂停门控) ── 依赖 Step 5
            └─ Step 7 (API: pause/resume) ── 依赖 Step 5, 6
                 └─ Step 8 (启动决策) ── 依赖 Step 6
                      └─ Step 9 (Dashboard 开关) ── 依赖 Step 7, 8（复用 Step 4 的 B5 预留）
Step 10 (文档同步) ── 依赖 Step 2-9 全部完成后
Step 11 (附项 A-D) ── 任意位置，各自独立
```

**关键约束：**
- Step 1-9 按编号线性串行执行（先测试功能后全局开关；Phase 0 全部完成后再进入 Phase 1-5）
- Step 5-8 构成全局开关主干，必须严格串行（数据层 → 门控 → API → 启动）
- Step 11 各项互相独立，可穿插执行，不作为主链路验收条件
- 每个 Step 完成后必须执行对应验收命令，全部通过才可进入下一步
...
#### 1.1 `webui/frontend/src/api.ts`（新增）—— 统一 fetch 封装
...
要点：
- 所有非 2xx 响应统一抛 `RequestError`（携带 `data.error`），修复现有"失败误报成功"
- `timeoutMs` 可选：连接测试传 `15000`；`AbortError` 统一转为"请求超时"文案（供连接测试超时分支使用）
- 未知 API 路径返回 HTML 时 `res.json()` 解析失败被 catch，不会抛出原始解析错误

#### 1.2 `webui/frontend/src/constants.ts`（新增）—— 共享常量
...
（消除 `Targets.vue` 与 `Advanced.vue` 的两处重复定义；`Targets.vue` 的列渲染改引 `cloudLabelMap`。）

#### 1.3 `webui/frontend/src/types.ts` —— 更新
...
export interface DryRunResponse {        // 响应包装（Step 2 后端配套）
...
#### 1.4 `webui/frontend/src/composables/useDryRun.ts`（新增，`composables/` 为新建目录）—— Dry Run 共享组合逻辑
...
### Step 2：后端 Dry Run 升级 + 连接测试凭据提示

**目标：** `POST /api/sync/dryrun` 响应结构升级（方案 A：明细化 + 包装对象），补齐限速/快照锁/防重入；连接测试凭据空值快速失败。

**前置条件：** Step 1 完成（`types.ts` 契约先行）

**产出文件与操作：**

#### 2.1 `provider/common.go` —— `RuleChange` 摘要类型与构造函数
...
#### 2.2 `syncer/syncer.go` —— `DryRun()` 升级（明细化 + 限速 + 快照锁 + 防重入 + warnings）
...
// Syncer 结构体新增字段：
// dryRunMu sync.Mutex  // DryRun 防重入
...
func (s *Syncer) DryRun() (DryRunResponse, error) {
	...
	// 快照：RLock 保护 providers/cfg，期间热重载不影响本次结果（Design2 §9.2）
	s.mu.RLock()
	...
	for _, p := range providers {
		...
		for _, rule := range rules {
			...
			resp.Results = append(resp.Results, result)
			time.Sleep(rateLimitInterval(p.CloudType())) // 限速：与 syncAll 一致（Design2 审查发现）
		}
	}
	return resp, nil
}
...
要点：
- **限速**：循环内复用 `rateLimitInterval(p.CloudType())`（对照 `syncAll()` 的间隔，遵守 AGENTS.md §七）
- **快照锁**：`s.mu.RLock()` 读取 `providers`/`cfg`；`ReloadProviders` 已有写锁，配对成立。**同时建议**：`Run()` 中 `s.cfg = newCfg` 的替换改为在 `s.mu.Lock()` 下进行（一行改动，消除 cfg 读取竞态）
- **防重入**：`dryRunMu.TryLock()`（Go 1.18+），冲突返回 `ErrDryRunInProgress` → handler 转 409
- **warnings**：在 Syncer 层生成（handler 只透传），语义见 Design2 §6.6
- 空结果时 `Results` 初始化为 `[]` 而非 `nil`（保证 JSON 输出 `[]`，前端可正常渲染空状态）

#### 2.3 `webui/api/sync.go` —— `handleSyncDryRun` 透传包装 + 409
...
#### 2.4 `webui/api/targets.go` —— 凭据空值快速失败（+4 行 + import）
...
`syncer/syncer_test.go` 新增（新建文件，包内定义 stub Provider 或复用真实 Diff）：
- `TestDryRun_EmptyConfig`：无 providers/无规则 → `Warnings` 两条、`Results` 为空数组
- `TestDryRun_Detail`：构造单域名规则 + stub resolver（解析 `localhost`），断言 `ToAdd`/`ToDelete` 明细数组长度与字段
...
#### 3.1 `webui/frontend/src/components/DryRunResults.vue`（新增，`components/` 为新建目录）—— 三级分组视图 + 空状态
...
- 分组渲染（三级：目标 → 域名 → 规则）：
  ...
  - `result.error` 非空 → 显示 `NAlert type="error"` 错误行，不展开明细
- 统计条（由父级传入或内部计算）：目标数 / 待添加总数 / 待删除总数 / 错误数（`NStatistic` 或 `NTag` 组合）

#### 3.2 `webui/frontend/src/views/RunTest.vue`（新增）—— 页面骨架
...
└── NTabPane "connection"（tab="连接测试"）
    ...
    └── 结果区：data.success ? data.message : data.error（200+success:false 语义保留）
         超时：request('/api/test-connection', {...}, 15000) 抛"请求超时"→ 显示"连接超时（15 秒），请检查网络或云 API 状态"
```

要点：
- 连接测试表单**内嵌本文件**（约 40 行），不抽 composable（Design2 §8.5 决策）
- `cloudOptions` 从 `constants.ts` 引用
- Tab 切换用 `useRoute`/`router.replace` 同步 `?tab=` query，刷新/分享保留位置

#### 3.3 `main.ts` / `App.vue` —— 路由与菜单
...
### Step 4：页面收敛（全站接入 `api.ts` + 删除高级功能页）
...
#### 4.1 `views/Settings.vue`
- `save()` 改走 `request('/api/settings', { method: 'PUT', body: ... })`，失败时 `message.error(e.message)`（修复误报成功）
- 保留现有「导出配置」「导入配置」按钮（唯一入口，Design2 §8.6）；可选微调：按钮行下加说明文案"导入会覆盖当前配置"

#### 4.2 `views/Targets.vue`
...
#### 4.5 `views/Dashboard.vue`（B5 预留）
- 删除：`dryRun()`、`dryrunResults`、`showDryrun`、`dryrunColumns`、`NModal` 及相关 import
- 新增：「运行测试」次级链接（`NButton` 或 `<router-link>`，放「同步引擎」状态卡片内）
- **B5 预留**：操作区渲染数据驱动——`status.enabled === false` 时「立即同步」`disabled` + hover 提示"请先开启同步引擎"（后端未返回 `enabled` 时默认 true，行为不变）

#### 4.6 删除高级功能页
- 删除 `views/Advanced.vue` 文件
- `main.ts` 删除路由 `{ path: '/advanced', ... }`
- `App.vue` 菜单删除「高级功能」项
- 全库检索 `/advanced` 引用并清理（`grep -rn advanced webui/frontend/src`）
...
# 手动验证：保存目标/规则失败时出现错误提示而非成功提示；仪表盘无试运行按钮，有「运行测试」链接
```

---

### Step 5：数据层 —— `Config.SyncEnabled` 持久化

**目标：** `sync_enabled` 键（SQLite）与 `SYNC_ENABLED` 环境变量（.env）读写。

**前置条件：** 无（与 Step 1-4 无依赖，但按顺序执行）
...
#### 5.2 `config/store.go` —— `LoadConfig()` 新增解析（默认值 + 读取）
...
#### 5.3 `config/env.go` —— `ParseEnv()` 新增解析
...
#### 5.4 单元测试（`config/env_test.go`）

新增用例：
- `SYNC_ENABLED=true` → `cfg.SyncEnabled == true`
- `SYNC_ENABLED=false` → `cfg.SyncEnabled == false`
- 未设置 → 默认 `true`
- 非法值（如 `abc`）→ 按 `false` 处理（`v == "true"` 语义，文档注明）
...
### Step 6：Syncer 暂停门控（Pause Gate + 热重载开关同步）
...
`New()` 中初始化：`syncEnabled: cfg.SyncEnabled`、`pauseCh: make(chan struct{}, 1)`、`resumeCh: make(chan struct{}, 1)`。

#### 6.2 `Run()` 主循环改造（启动门控 + 暂停子循环）
...
func (s *Syncer) Run() {
	...
	for {
		...
		case newCfg := <-s.configCh:
			slog.Info("配置热重载")
			s.mu.Lock()
			s.cfg = newCfg // 加锁替换（消除与 DryRun 快照的 cfg 竞态）
			s.mu.Unlock()
...
> **设计说明（合理化完善）：** Design2 §5.2 原设计为"暂停子循环 + 主循环分置"；本 Step 将其整合为单一 `waitForResume()`（Run 启动时暂停与运行中暂停共用），并**在暂停子循环内同步监听 `configCh`**（热重载开启可立即恢复），同时实现 §5.6 的开关同步——保证「DB → cfg → 运行时镜像 → 门控」一致。`Run()` 启动时若 `syncEnabled=false` 直接进入 `waitForResume()` 且**不执行首次 syncAll()**（对应 Design2 §7.1 "始终启动 Run()"决策，`.env` 模式 `doneCh` 正常关闭，`WaitForSignal` 无需特殊处理）。
...
> **并发说明：** `Pause()`/`Resume()` 直接赋值 `syncEnabled`（主循环外唯一写者场景），与 `waitForResume()` 的读取存在理论竞态；为简单起见采用与 §5.5 一致的容量 1 channel + `select/default`，赋值在信号发送前完成、信号由主循环串行消费，可接受（符合"不过度防御"）。若 `go test -race` 报告，可将 `syncEnabled` 改为 `atomic.Bool`。

#### 6.4 单元测试（`syncer/syncer_test.go` 新建）

使用 stub Provider（实现 `provider.Provider` 接口，`GetRules` 返回空）与真实 `dns.Resolver`（解析 `localhost`）：
- `TestRun_DisabledStartup`：`SyncEnabled=false` 创建 Syncer → `Run()` 后不执行同步（stub 计数为 0）→ `Resume()` 后执行一次（计数 ≥1）→ `Stop()`
- `TestPauseResume_Flow`：正常运行 → `Pause()` → 计数停止增长 → `Resume()` → 计数恢复
- `TestReload_SyncEnabledSync`：运行中 `Reload(SyncEnabled=false 的 cfg)` → 门控生效（计数停止）；再 `Reload(SyncEnabled=true)` → 恢复
...
### Step 7：API 层 —— pause/resume 端点 + trigger 保护 + SyncStatus.enabled

**目标：** 新增 `POST /api/sync/pause|resume`；暂停时 trigger 返回 409；`SyncStatus` 带 `enabled`。

**前置条件：** Step 5、Step 6 完成

**产出文件与操作：**

#### 7.1 `webui/api/deps.go` —— Syncer 接口扩展
...
#### 7.2 `syncer/syncer.go` —— `SyncStatus` 扩展
...
#### 7.3 `webui/api/sync.go` —— 新增 handler + 路由 + trigger 保护
...
#### 7.4 单元测试（`webui/api/sync_test.go` 新建，可选但推荐）

使用 stub Syncer（实现接口） + `httptest`：
- `TestHandleSyncTrigger_Paused`：stub `Status().Enabled=false` → 断言 409 + `{"error":"同步已暂停，请先开启"}`
- `TestHandleSyncPause`：stub Store（或真实 Store + 临时 SQLite 文件）→ 断言 200 且 settings 表写入 `sync_enabled=false`

**验收：**
```bash
go build ./... && go vet ./... && go test ./...
# 手动：WebUI 启动后 curl -X POST /api/sync/pause 与 /api/sync/resume 验证响应
```

---

### Step 8：启动决策统一（始终启动 `Run()`）

**目标：** 确认/落地 Design2 §7.1/§7.2 决策——`main.go` 与 `app.go` **不做条件分支**，`go s.Run()` 保持原样，启动门控由 Step 6 的 `Run()` 内部完成。

**前置条件：** Step 6 完成

**产出文件与操作：**

- `main.go`：`go s.Run()` 保持不变（零改动或仅注释说明：启动门控在 `Run()` 内，见 Step 6.2）。删除任何计划中的 `if cfg.SyncEnabled` 分支（当前代码无此分支，确认即可）
- `app/app.go`：`go s.Run()` 保持不变（`.env` 模式同样受益：`SYNC_ENABLED=false` 时 `Run()` 存活于 `waitForResume()`，`doneCh` 正常关闭，`WaitForSignal()` 无需适配）
- 验证 `main.go` WebUI 模式流程：`LoadConfig()`（含 `SyncEnabled`）→ `syncer.New(cfg, ...)`（`syncEnabled` 镜像初始化）→ `srv.SetSyncer(s, ...)` → `go s.Run()`

**测试与验收：**
```bash
go build ./... && go vet ./... && go test ./...
# 双模式运行验证：
# 1) WebUI 模式：FWALIZER_MODE=webui ./fwalizer → 正常启动，无 TARGETS 时进入等待配置状态
# 2) .env 模式：临时 .env 加 SYNC_ENABLED=false → 启动后日志出现"同步已暂停"，Ctrl+C 正常退出（无死锁）
# 3) 恢复 SYNC_ENABLED=true → 启动后立即执行首次 syncAll
```

---

### Step 9：Dashboard 同步开关 + 按钮状态联动

**目标：** 仪表盘暂停/开启开关、三态状态标签、「立即同步」联动（Design2 §8.1-8.3，复用 Step 4 的 B5 预留）。
...
UI 变更：
- 「同步引擎」状态标签三态（Design2 §8.2）：
  - `enabled === true` → 绿色 success「运行中」
  - `enabled === false` → 橙色 warning「已暂停」（说明：`running` 保持引擎存活语义，见 Design2 §8.2 说明）
- 操作卡片：新增 `NButton`「暂停」/「开启」（`:loading="switching"`）；「立即同步」在 `enabled === false` 时 `disabled` + tooltip"请先开启同步引擎"
- 保留「运行测试」链接（Step 4 已加）

`types.ts`：`SyncStatus.enabled` 由可选字段升为必填（后端已返回；如仍兼容旧后端可保留 `?`）
...
cd ../.. && go build ./... && go vet ./... && go test ./...
# 手动验证：
# 1) 点击「暂停」→ 标签变「已暂停」、「立即同步」置灰、API 返回 200
# 2) 暂停状态下 POST /api/sync/trigger 返回 409
# 3) 点击「开启」→ 立即执行一次同步（同步日志出现记录）
# 4) 重启应用 → 开关状态保持（SQLite 持久化）
```
...
| `Design2.md` | 12.3 验收标准全部满足后，将状态标记为已实施（若文档体系要求） |

**测试与验收：**
```bash
grep -n "9090" Design1.md AGENTS.md README.md .env.example || echo "端口引用一致"
...
### Step 11：低优先可选附项（Design2 §12.4，可独立执行）

> 每项相互独立，不阻塞主链路验收；按需选择执行。

#### 附项 A：`INTERVAL` 解析校验统一
- **现状**：`env.go` 解析失败返回错误；`store.go` `LoadConfig` 静默 fallback 5m
- **方案**：`LoadConfig` 中 `interval` 解析失败时 `slog.Warn` 并保留默认值（不打断启动），`Settings.vue` 的 interval 输入加 `placeholder="5m"` 与保存前格式校验（正则 `^\d+(ms|s|m|h)$`）
- **验收**：`go build ./...`；设置页输入非法间隔保存时提示错误

#### 附项 B：未知 API 路径返回 HTML（验证项，无代码改动）
- **现状**：`server.go` 的 `Handle("/")` 兜底，未知 `/api/*` 返回 SPA
- **结论**：Step 1 的 `api.ts` 已天然免疫（`res.json()` 失败进 catch），无需改动
...
- **方案**：ReloadFunc 收集失败目标列表，失败数量 > 0 时 `slog.Error("部分目标重建失败", "failed", n)`；后续可扩展为 API 暴露
- **验收**：`go build ./...`；构造非法目标后修改配置触发热重载，日志含失败汇总

#### 附项 D：`Settings.vue` 补 `dns_fail_threshold` 输入框
- **现状**：后端 `LoadConfig` 已支持该 key，前端表单缺失
- **方案**：`settings.go` 的 defaults map 增加 `"dns_fail_threshold": "5"`；`Settings.vue` 在 `dns_timeout` 下新增输入框（NInput，placeholder "5"）
...
| Design2 §12.3 验收条目 | 对应 Build3 Step |
|------------------------|------------------|
| 1-8（全局开关 8 条） | Step 5、6、7、8、9 |
| 9-14（Dry Run 明细/分组/空状态/错误/防连点/限速） | Step 2、3 |
| 15（入口合并） | Step 3、4 |
| 16-18（连接测试） | Step 2、3、4 |
| 19（页面收敛） | Step 4 |
| 20（开关状态一致） | Step 6（5.6 实现） |
| 21（统一封装） | Step 1、4 |
...
1. **Step 6 是核心复杂度所在**：`Run()` 主循环重构涉及并发路径，务必先写测试再改实现（测试先行），并运行 `-race`
2. **Step 2 的 `DryRun()` 签名变更**会影响 `deps.go` 接口与 Step 1 的前端契约，前后端必须同一轮完成，避免中间态
3. **Step 4 删除 Advanced 页前**先 `grep -rn advanced` 确认无残留跳转引用
4. **Step 9 的 `enabled` 语义**：`running` 保持"引擎存活"语义（`setRunning` 在 Run 启停时切换），前端三态判断以 `enabled` 为准（Design2 §8.2 说明）
5. **Step 11 附项 C** 涉及 ReloadFunc（`main.go`），改动时注意告警订阅重建顺序不受影响
