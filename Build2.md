...
| Step | 编号 | 内容 | 严重度 | 状态 |
|------|------|------|--------|------|
| 1 | R11-01 | `.dockerignore` 去重 | 🟡 中 | ☐ 未开始 |
| 2 | R11-02 | 配置导入事务保护 | 🔴 高 | ☐ 未开始 |
| 3 | R11-03 | 前端数组索引→DB ID | 🔴 高 | ☐ 未开始 |
| 4 | R11-04 | CI/CD 前端构建步骤 | 🔴 高 | ☐ 未开始 |
| 5 | R11-05 | .env 模式 0-based 规则过滤 | 🟡 中 | ☐ 未开始 |
| 6 | R11-06 | 移除 `app.Run` mode 参数 | ⚪ 低 | ☐ 未开始 |
| 7 | R11-07 | README DNS 默认值同步 | ⚪ 低 | ☐ 未开始 |
| 8 | WEB-06 | `/advanced` + `/alerts` 页面 + 告警 API | 🟡 中 | ☐ 未开始 |
| 9 | FEA-02 | 告警通知器接入 EventBus | 🟡 中 | ☐ 未开始 |
| 10 | FEA-03 | CLI `backup` / `restore` | 🟡 中 | ☐ 未开始 |
| 11 | FEA-06 | systray 同步触发 + 开机自启 | ⚪ 低 | ☐ 未开始 |
